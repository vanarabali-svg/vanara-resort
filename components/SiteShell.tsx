...
export default function SiteShell({ lang, children }: { lang: Locale; children: React.ReactNode }) {
  ...
  useEffect(() => { document.body.classList.add('site-preload') }, [])
  ...
  useEffect(() => {
    ...
    return () => document.body.classList.remove('site-preload')
  }, [loadingVisible])
  ...
  return (
    <>
      ...
      <div className={`siteAppShell ${loadingVisible ? 'is-hidden' : ''}`}>
        <main>{children}</main>
        ...
        {!cookieOpen && cookieReady && hydrated && <button type="button" className="cookieManageButton" onClick={() => setCookieOpen(true)}>{t.layout.cookieEyebrow}</button>}
