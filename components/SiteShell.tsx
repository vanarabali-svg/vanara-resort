...
export default function SiteShell({ lang, children }: { lang: Locale; children: React.ReactNode }) {
  ...
  const cookieConsentKey = 'vanara-cookie-consent-v5'
  ...
  return (
    <>
      ...
      {cookieReady && cookieOpen && <div className="cookiePopup" role="dialog" aria-live="polite" aria-label={t.layout.cookieEyebrow}><div className="cookiePopupInner"><div className="cookiePopupEyebrow">{t.layout.cookieEyebrow}</div><div className="cookiePopupTitle">{t.layout.cookieTitle}</div><p className="cookiePopupText">{t.layout.cookieText}</p><div className="cookiePopupActions"><Link className="cookiePopupLink" href={withLang(lang, '/privacy')}>{t.layout.readPolicy}</Link><button type="button" className="cookiePopupButton cookiePopupButtonGhost" onClick={() => saveCookieConsent('declined')}>{t.layout.decline}</button><button type="button" className="cookiePopupButton" onClick={() => saveCookieConsent('accepted')}>{t.layout.accept}</button></div></div></div>}


          <footer className="footer"><div className="footer-inner"><div className="footer-brand"><div className="footer-name">Vanara Resort &amp; Spa</div><div className="footer-sub">Uluwatu, Bali</div><div className="footer-social" aria-label={t.layout.social}><a href="https://www.instagram.com/vanararesort/" target="_blank" rel="noreferrer" aria-label="Instagram" className="social-btn"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5" /><circle cx="12" cy="12" r="4" /><circle cx="17.5" cy="6.5" r="1" /></svg></a><a href="#" aria-label="Facebook" className="social-btn"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 9h3V6h-3c-2 0-4 2-4 4v3H7v3h3v5h3v-5h3l1-3h-4v-3c0-.6.4-1 1-1z" /></svg></a><a href="https://www.pinterest.com/vanarabali/" target="_blank" rel="noreferrer" aria-label="Pinterest" className="social-btn"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3.5c-4.2 0-7.5 3.1-7.5 7.3 0 3 1.7 5.2 4.2 6.1-.1-.5-.2-1.4 0-2l1.1-4.4s-.3-.6-.3-1.5c0-1.4.8-2.5 1.9-2.5.9 0 1.3.7 1.3 1.5 0 .9-.6 2.3-.9 3.6-.2 1 .5 1.9 1.5 1.9 1.8 0 3-2.4 3-5.3 0-2.2-1.5-3.9-4.2-3.9-3.1 0-5 2.3-5 4.8 0 .9.3 1.8.8 2.4.2.2.2.3.1.6l-.3 1.2c-.1.4-.4.5-.7.4-1.9-.8-3.1-3-3.1-5.4 0-3.6 3.4-7.9 9.1-7.9 4.6 0 7.6 3.3 7.6 6.8 0 4.7-2.6 8.2-6.4 8.2-1.3 0-2.4-.7-2.8-1.4l-.8 3c-.3 1.1-.8 2.1-1.3 2.9.9.3 1.8.4 2.8.4 4.2 0 7.5-3.1 7.5-7.3S16.2 3.5 12 3.5z" /></svg></a></div></div><div className="footer-col"><div className="footer-head">{t.layout.footerContact}</div><div className="footer-text">VANARA Resort &amp; Spa</div><div className="footer-text">PT. UWSUN ECO RESORT</div><div className="footer-text">Jl. Batu Nunggalan No.9, Pecatu, Uluwatu, Bali 80361</div><div className="footer-text">Phone / WhatsApp: <a className="footer-link" href="https://wa.me/628135356240">+62 813 5356 240</a></div><a className="footer-link" href="mailto:info@vanara.life">Email: info@vanara.life</a></div><div className="footer-col"><div className="footer-head">{t.layout.footerQuickLinks}</div><Link className="footer-link" href={withLang(lang, '/about')}>{t.menu.about}</Link><Link className="footer-link" href={withLang(lang, '/accommodation')}>{t.menu.villas}</Link><Link className="footer-link" href={withLang(lang, '/kokoon')}>{t.layout.dining}</Link><Link className="footer-link" href={withLang(lang, '/experience')}>{t.layout.experiences}</Link><Link className="footer-link" href={withLang(lang, '/weddings')}>{t.layout.weddings}</Link><Link className="footer-link" href={withLang(lang, '/connect')}>{t.menu.connect}</Link></div><div className="footer-col"><div className="footer-head">{t.layout.footerInformation}</div><Link className="footer-link" href={withLang(lang, '/terms')}>{t.layout.terms}</Link><Link className="footer-link" href={withLang(lang, '/privacy')}>{t.layout.privacy}</Link><Link className="footer-link" href={withLang(lang, '/legal')}>{t.layout.legal}</Link></div></div><div className="footer-copy">© {new Date().getFullYear()} Vanara Resort &amp; Spa</div></footer>
    </>
  )
}
